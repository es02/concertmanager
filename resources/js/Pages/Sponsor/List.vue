...
const props = defineProps(['sponsors', 'count']);
...
const numberOfPages = Math.ceil(props.count / sponsorsPerPage);

const paginatedSponsors = computed(() => {
    var start = (currentPage.value - 1) * sponsorsPerPage;
    ...
    if (props.sponsors.data) {
        return props.sponsors.data;
    }
    return props.sponsors.slice(start, end);
});

function changePage(page){
    console.log(page);
    var pageSort = sortByURL.value;
    if (props.sponsors.path && props.sponsors.path.split('/').slice(-1)[0] == 'sponsor') {
        // assume search
        router.get(props.sponsors.path.concat(`?page=${page}`));
    } else {
        router.get(`/sponsors/${page}${pageSort}`);
    }
}
...
function uploadCSV() {
    ...
    router.post('/sponsors/import-csv', form);
    form.import_csv = '';
    csvInput.value.value = null;
};

function downloadCSV() {
    window.location.href = '/sponsors/export-csv';
};
...
function goToCreateSponsor() {
    router.visit(route('sponsor.create'));
};
...
<template>
    <AppLayout title="Sponsors">
        <h1 class="text-2xl font-semibold text-center pt-10">Sponsors</h1>
        <div class="overflow-x-auto m-10 text-right">
            <!--<search-input route-name="sponsor.search" />-->
            <button type="submit" @click="goToCreateSponsor()" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">New Sponsor</button>
        ...
        <div class="overflow-x-auto m-10">
            ...
            <table class="table">
                <thead>
                    <tr>
                        <th><a :href="`/sponsors/${currentPage}/name`">Name</a></th>
                        <th><a :href="`/sponsors/${currentPage}/location`">Location</a></th>
                        <td></td>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="sponsor in paginatedSponsors" :key="sponsor.id">
                        <td class="link link-primary">
                            <Link :href="`/sponsor/${sponsor.id}`">{{ sponsor.name }}</Link>
                        </td>
            ...
            <div class="join flex justify-center fixed w-full p-4">
                ...
                <ul class="inline-flex -space-x-px text-sm">
                    <li>
                        ...
                        <a v-else :href="`/sponsors/${currentPage - 1}`" class="flex items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</a>
                    </li>
                    <li v-for="page in numberOfPages">
                        ...
                        <a v-else :href="`/sponsors/${page}`" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">{{ page }}</a>
                    </li>
                    <li>
                        ...
                        <a v-else :href="`/sponsors/${currentPage + 1}`" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</a>
                    </li>
                </ul>
            </nav>
            </div>
